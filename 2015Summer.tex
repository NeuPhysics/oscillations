...
The only part inside $\mathbf{U_x(x)}$ that is space dependent is the number density of the electrons $n(x)$. {\bf{Thus we know immediately that the Hamiltonian is diagonalized if the number density is constant.}} \footnote{But even the electrons density is constant, the oscillation is different.}\footnote{\bf It would be nice to find the expression of $\mathbf{U_x}$ in order to play with the matter effect.}\footnote{In a two level system, suppose the Hamiltonian is
...
Suppose we have an environment with constant electron number density, the term $- i \mathbf{U_x^{-1}} ( \partial_x \mathbf{U_x} )$ goes away. All we have is the diagonalized new Hamiltonian $\mathbf{H_{xd}}$ and the eigenvalues are easily obtained which are
...
\subsection{Solar Electron Number Density}
