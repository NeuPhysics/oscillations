...
The effective potential is\footnote{This expression comes from the result of the second order approximation of $\sin\theta(x)$ and $\cos\theta(x)$. There is a reason that this expression $\mathbf{U_m^{-1}}\partial_x \mathbf{U_m}$ has only off-diagonal elements.}
...
\begin{equation*}
\delta \omega_m =  \omega_m(x_r) +  \alpha (x-x_r),
\end{equation*}
...
\begin{equation*}
\gamma  = \frac{v^2}{\left\vert \frac{d\omega_m}{dx}\right\vert_{x_r} }.
\end{equation*}
...
&P(x\to \infty, \ket{\nu_L}\to\ket{\nu_H})\\
=& e^{-2\pi\gamma} \\
 =& \exp\left( -2\pi \frac{v^2}{ \left\vert \frac{d\omega_m}{dx} \right\vert_{x_r} } \right) \\
 =& \exp\left( -2\pi \frac{\left( \cot\theta_v \frac{d\hat\Delta}{dx}  /4\right)^2}{\left\vert \frac{d\omega_m}{dx} \right\vert_{x_r} } \right)
\end{align*}

We have

\begin{equation*}
\left.\frac{d \delta \omega_m}{dx}\right\vert_{x_r} = \left.\frac{d\hat\Delta}{dx} \frac{d\delta \omega_m}{d\hat\Delta} \right\vert_{x_r} = \hat\Delta'(x_r) \omega \sin\theta_v 
\end{equation*}

The final result should be (which is different from the result I have)

\begin{align*}
P(x\to \infty, \ket{\nu_L}\to\ket{\nu_H}) 
& = 
\exp \left( -\frac{\pi}{2}\frac{\sin^2 2\theta_v}{\cos 2\theta_v} \frac{\omega}{\lvert  \frac{d\hat\Delta}{dx} \vert_{x_r} \rvert} \right)
\end{align*}
