...
Since we are dealing with resonance which is located at $\hat\Delta =1$, the quantities can be expanded around $\hat\Delta - = 0$.

The series of them are\footnote{Assuming $\sin\theta$ is not negative.}

\begin{align*}
\sin 2\theta(x) & = \sin 2\theta_v \left( \frac{1}{ 2\sin\theta_v } - \frac{1}{4\sin\theta_v} (\hat\Delta - 1)  + \mathcal{O}(\hat\Delta -1)^2 \right) \\
& = \cos\theta_v - \frac{\cos\theta_v}{2}(\hat\Delta - 1) +\mathcal{O}(\hat\Delta - 1)^2 \\
\cos 2\theta(x) & = -\sin\theta_v - \frac{\cos\theta_v}{2\tan\theta_v} (\hat\Delta - 1)+ \mathcal{O}(\hat\Delta -1)^2 .
\end{align*}

Notice that for small $\theta_v$ the first order can be neglected sine they are second or higher order small quantities.

$\sin\theta(x)$ and $\cos\theta(x)$ can be found using these results up to second order.\footnote{We need second order because we are going to take the derivative of x. However, in this special case, we can directly solve out $\sin\theta(x)$ and $\cos\theta(x)$ using by plug the approximation $\hat\Delta(x) = 1+\hat\Delta'(x_r)(x-x_r)$ into the expression of $\sin 2\theta(x)$ and $\cos 2\theta(x)$ and WITHOUT Taylor expand $\sin 2\theta(x)$ and $\cos 2\theta(x)$.}


To find out the solutions the explicit expression for $\mathbf{U_m}$ is required.\footnote{\begin{equation*}
\mathbf{U_m} = \begin{pmatrix} \cos \theta(x) & \sin\theta(x) \\ -\sin\theta(x) & \cos\theta(x) \end{pmatrix} 
\end{equation*}} This can be done using Mathematica along with the derivative of this rotation matrix with respect to $x$ where a trick
...
The effective potential is\footnote{This expression comes from the result of the second order approximation of $\sin\theta(x)$ and $\cos\theta(x)$.}

\begin{align*}
\mathbf{V_m} & = -i\mathbf{U_m^{-1}} ( \partial_x \mathbf{U_m} ) \\
& = - i \frac{\hat\Delta'(x_r) \sin 2\theta_v}{ 2( 2+\hat\Delta'(x)(x-x_r) ( 2+ \hat\Delta'(x)(x-x_r)) ) - 2( 1 + \hat\Delta'(x)(x-x_r) ) \cos 2\theta_v }   \begin{pmatrix}
0 & 1 \\
-1 & 0
\end{pmatrix} .
\end{align*}

To keep only first order\footnote{\begin{equation*}
...
\begin{align*}
\mathbf{V_m} & = - i \hat\Delta'(x_r) \frac{\cot\theta_v}{4} \left( -1 + \hat\Delta'(x_r)(x-x_r)  \right)  \begin{pmatrix}
0 & 1 \\
-1 & 0
\end{pmatrix}.
\end{align*}
...
\begin{align*}
v &= -  \hat\Delta'(x_r) \frac{\cot\theta_v}{4} \left( -1 + \hat\Delta'(x_r)(x-x_r)  \right)  \\
...
Plug the state $\ket{\Psi_m}$ into the Schr\"odinger equation, we have
...
The boundary condition for this problem {\bf{in general}} is that

\begin{equation*}
\ket{\Psi_m(0)} = C_L(0)\ket{\nu_L} + C_H(0) \ket{\nu_H}.
\end{equation*}

{\bf{It should be made clear that the problem we will be discussing is the transition from one state to another in first order approximation. That means we will confine this system so that the initial condition is $\ket{\Psi_m(-\infty)} = \ket{\nu_L}$.}} In terms of $C_L$ and $C_H$,
...
\sout{Furthermore, the eigenvalues are not varying very fast and satisfies the condition that}

\begin{equation*}
\delta \omega_m = \alpha x,
\end{equation*}

\sout{where $\alpha$ is a constant and comes from the first order of the expression.}




\sout{Assuming one of the solutions is $e^{rx}$, we could insert it to the second order differential equation and get the characteristic equation,}

\begin{equation*}
r^2 - i\delta \omega_m r + v^2 = 0,
\end{equation*}

\sout{which has two solutions}
...
\sout{The solutions to $C_L(x)$ and $C_H(x)$ will be}
...
\sout{These constants are determined by the initial condition. The state of the system at any $x$ is}
...
\sout{{\color{red}Doesn't make sense.}}


{\bf{In the paper by Zener,}} we need to do substitution of the function $C_L$ and reduce the equation to Weber equation.\cite{Zener1932} 

Define a new variable $W$ which is determined by\footnote{This is done by assuming $C_L=f(x)W$ and plugging it back to the equation then set the coefficient of $\dot C_L$ to $0$.}
...
\begin{equation*}
\ddot W + \left( v^2 + \frac{i \alpha}{2} + \frac{\alpha^2 x^2}{4}  \right) W = 0,
\end{equation*}

which can be reduced to the standard form of Weber equation with the new parameters\footnote{They are found using a single assumption that $z=gx$.}
...
where $g^2\equiv -i\alpha$ ($g=(1-i)\sqrt{\alpha} /\sqrt{2}=\sqrt{\alpha}e^{-i\pi/4}x$). The equation we need to solve becomes
...
\begin{figure}
\centering
\includegraphics{assets/weber1.png}
\caption{The parabolic cylinder function $D_\nu(z)$ for $\nu=1$ (blue) and $\nu=2$ (red). But for imaginary $z$ the function blows up.}
\end{figure}

The Weber equation has two independent solutions $D_\nu(z)$ and $D_{-\nu-1}(iz)$.\footnote{They are alled Parabolic Cylinder Function on wolfram mathworld. \url{http://mathworld.wolfram.com/ParabolicCylinderFunction.html}}


Since $D_{\nu}(z)$ blows up for complex $z\propto e^{-\pi i/4}$, the solution that works is $D_{-\nu-1}(iz)$. Then the solution to $U_L$ is
...
Apply the boundary condition we have the results of the coefficients\footnote{The details depends on the signs of $\alpha$.}
...
What we need to find out is the state at $t\to \infty$, which depends on the asymptotic values of $D_{-\nu-1}$,
...
The transition rate is determined by $\lvert C_L \rvert^2$,\cite{Zener1932}

\begin{equation*}
\lvert C_L(\infty) \rvert = \gamma e^{-\pi\gamma} \frac{2\pi}{\Gamma(i\gamma +1) \Gamma(-i\gamma +1)} = 2e^{-\pi\gamma}\sinh \pi\gamma = 1-e^{-2\pi\gamma}.
\end{equation*}
...
\begin{equation*}
\gamma  = \frac{v^2}{\lvert \frac{d\omega_m}{x dx}\rvert_{x_r} }.
\end{equation*}
...
&P(x\to \infty, \ket{\nu_L}\to\ket{\nu_H})\\
=& e^{-2\pi\gamma} \\
 =& \exp\left( -2\pi \frac{v^2}{ \lvert \frac{d\omega_m}{xdx} \rvert_{x_r} } \right) \\
 =& \exp\left( -2\pi \frac{\left( -\sin\theta_v\cos\theta_v \frac{d\hat\Delta}{dx} \omega \right)^2}{\lvert \frac{d\omega_m}{xdx} \rvert_{x_r} } \right)\\
 =& \exp\left(  -\frac{\pi}{2} \frac{\sin^2 2\theta_v \omega}{ \cos 2\theta_v \lvert \frac{d \hat\Delta}{dx}/\hat\Delta  \rvert_{x_r}  }   \right)
\end{align*}
...
P(x\to \infty, \ket{\nu_L}\to\ket{\nu_H}) 
& = 
\exp \left( -\frac{\pi}{2}\frac{\sin^2 2\theta_v}{\cos 2\theta_v} \frac{\omega}{\lvert \frac{1}{\hat\Delta} \frac{d\hat\Delta}{dx} \vert_{x_r} \rvert} \right)
\end{align*}

The survival probability can be calculated by counting the probability left on the initial state.

To be clear, if electron neutrinos are produced inside core of our sun, it will be the heavy state. Since the interaction with matter is very strong, it transfers to $\ket{\nu_L}$ with probability $P(x\to \infty, \ket{\nu_L}\to\ket{\nu_H}) $ due to the gradient of the matter profile which works as the perturbation. Thus the final state will be a mixing of $\ket{\nu_L}$ and $\ket{\nu_H}$.
...
{\color{red}This means that the adiabatic result is of the form
\begin{equation*}
